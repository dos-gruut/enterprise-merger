...
void BpScheduler::start() {
  // TODO: Merger id와  chain id에 따라 수정필요
  // setMyIds("MERGER_ID", "CHAIN_ID");
...
void BpScheduler::setMyIds(merger_id_type &mid, chain_id_type &cid) {
  m_my_mid = mid;
  m_my_mid_b64 =
      macaron::Base64::Encode(std::string(m_my_mid.begin(), m_my_mid.end()));
  m_my_cid = cid;
  m_my_cid_b64 =
      macaron::Base64::Encode(std::string(m_my_cid.begin(), m_my_cid.end()));
}
    ...
    if (my_prev_status == BpStatus::SECONDARY) {
      ...
      if (my_prev_status == BpStatus::PRIMARY) {
        ...
        if(list_size == 1)
          ...
          if (others_status == BpStatus::SECONDARY || others_status == BpStatus::PRIMARY)
            m_current_status = BpStatus::SECONDARY;
          else
            m_current_status = BpStatus::IDLE;
        }
...
void BpScheduler::runTxCollector() {}
